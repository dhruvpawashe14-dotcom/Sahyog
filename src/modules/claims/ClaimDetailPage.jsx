...
export default function ClaimDetailPage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      showToast("Couldn't save changes. Please try again or contact admin.", 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const changeStatus = async (status) => {
    try {
      ...
      showToast("Couldn't update claim status. Please try again or contact admin.", 'error');
      return;
    }
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      await logAudit({ userId: user.id, userName: profile.full_name, action: 'DELETE', module: 'Claims', recordId: id, details: `Claim deleted: ${claim.client_name}` });
    ...
    <div className="ticket-detail">
      <div className="ticket-main card">
        <div className="page-hdr">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {isAdmin && (
              <button className="btn" onClick={() => setConfirmDelete(true)} title="Delete claim" style={{ color: 'var(--red)' }}>
            ...
            {documents.length === 0 ? <div className="table-empty">No documents uploaded</div> :
              documents.map((d) => (
                <div key={d.id} className="dup-row">
                  ...
                  <SecureFileLink bucket="claim-documents" path={d.file_path} style={{ marginLeft: 10 }} />
                </div>
              ))}
          </div>
        )}
      ...
      >
        <p style={{ fontSize: 13, color: 'var(--text3)' }}>
          This permanently deletes <b>{claim.claim_ref}</b> — {claim.client_name} — along with its
          full activity timeline and uploaded documents. Any ticket linked to it stays, just unlinked.
