...
import { getSignedFileUrls } from '../../services/supabase/secureStorage';
...
export default function TicketDetailPage() {
  ...
  const load = async () => {
    ...
    const commentRows = await ticketService.listComments(id);
    setComments(commentRows);
    setParticipants(await ticketService.listTicketParticipants(id));
    // ticket-attachments is a private bucket — mint short-lived signed URLs for
    // every file/voice comment on load, rather than storing/reusing a
    // permanent link. 1hr expiry covers a normal viewing session.
    const filePaths = commentRows.filter((c) => c.is_file && c.file_path).map((c) => c.file_path);
    setFileUrls(await getSignedFileUrls('ticket-attachments', filePaths, 3600));
  };
  ...
  const sendFile = async (file) => {
    setUploading(true);
    try {
      const path = await ticketService.uploadTicketAttachment(id, file);
      await ticketService.sendComment({ ticketId: id, authorId: user.id, authorName: profile.full_name, body: file.name, isFile: true, filePath: path });
  ...
  const toggleVoiceReply = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          const path = await ticketService.uploadTicketAttachment(id, file);
          await ticketService.sendComment({ ticketId: id, authorId: user.id, authorName: profile.full_name, body: 'Voice reply', isFile: true, filePath: path, isVoiceNote: true });
  ...
  const logFollowUp = async () => {
    const count = await ticketService.logNewQuery({ ticketId: id, authorId: user.id, authorName: profile.full_name });
    await logAudit({ userId: user.id, userName: profile.full_name, action: 'UPDATE', module: 'Tickets', recordId: id, details: `Logged follow-up query #${count}` });
    await notifyTicketMembers(`New query on ${ticket.ticket_ref}`, `Follow-up #${count} received — clock reset`);
    showToast(`Logged as query #${count} — "open for" clock reset`, 'success');
    load();
  };
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      await logAudit({ userId: user.id, userName: profile.full_name, action: 'DELETE', module: 'Tickets', recordId: id, details: `Ticket deleted: ${ticket.subject}` });
    ...
    <div className="ticket-detail">
      <div className="ticket-main card">
        <div className="page-hdr">
          <div>
            <span className="ticket-ref">{ticket.ticket_ref}</span>
            {ticketService.hasFollowUps(ticket) && (
              <span title={`${ticket.query_count} queries logged on this ticket`} style={{ marginLeft: 8, fontSize: 11, fontWeight: 600, color: 'var(--gold)', border: '1px solid var(--gold)', borderRadius: 20, padding: '1px 8px' }}>
                Follow-up ×{ticket.query_count}
              </span>
            )}
            <h2>{ticket.subject}</h2>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn" onClick={logFollowUp} title="Client raised a new question on this open ticket">
              <i className="ti ti-message-circle-plus" /> New Query
            </button>
            <select value={ticket.status} onChange={(e) => changeStatus(e.target.value)}>
              {TICKET_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
            {isAdmin && (
              <button className="btn" onClick={() => setConfirmDelete(true)} title="Delete ticket" style={{ color: 'var(--red)' }}>
        ...
        {ticket.description && (
          <div style={{
            fontSize: 13.5, color: 'var(--text2)', background: 'var(--ivory3)',
            border: '1px solid var(--border2)', borderRadius: 8, padding: '10px 12px',
            marginBottom: 10, lineHeight: 1.5, whiteSpace: 'pre-wrap',
          }}>
            {ticket.description}
          </div>
        )}
        ...
        <div className="chat-messages">
          ...
          {comments.map((c) => (
            <div key={c.id} className={`chat-bubble ${c.author_name === profile.full_name ? 'mine' : ''}`}>
              <div className="chat-author">{c.author_name} <span style={{ color: 'var(--text4)', fontWeight: 400 }}>· {fmtTimestamp(c.created_at)}</span></div>
              {c.is_voice_note ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <i className="ti ti-microphone" style={{ color: 'var(--gold)' }} />
                  <audio controls src={fileUrls.get(c.file_path)} style={{ height: 32, maxWidth: 220 }} />
                </div>
              ) : c.is_file ? (
                <a href={fileUrls.get(c.file_path)} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'inherit' }}>
        ...
        <div className="chat-input-row">
          ...
          <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Type message... (Ctrl+Enter to send, or paste a screenshot)"
            onKeyDown={(e) => { if (e.ctrlKey && e.key === 'Enter') send(); }}
            onPaste={(e) => {
              const imageItem = Array.from(e.clipboardData.items).find((item) => item.type.startsWith('image/'));
              if (!imageItem) return; // let normal text paste through untouched
              e.preventDefault();
              const blob = imageItem.getAsFile();
              if (!blob) return;
              const ext = blob.type.split('/')[1] || 'png';
              const file = new File([blob], `pasted-screenshot-${Date.now()}.${ext}`, { type: blob.type });
              sendFile(file);
            }} />
      ...
      <div className="ticket-side">
        <div className="card">
          ...
          {ticketService.hasFollowUps(ticket) && (
            <div className="dup-row" style={{ fontSize: 11, color: 'var(--text4)' }}>
              Originally raised {ticketService.totalAgeLabel(ticket)} · {ticket.query_count} queries logged
            </div>
          )}
          ...
          <div className="dup-row">Raised on: {ticketService.fmtShortDate(ticket.created_at)}</div>
          ...
          {taggableEmployees.length > 0 && (
            <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
              <select value={addingParticipant} onChange={(e) => setAddingParticipant(e.target.value)} style={{ flex: 1 }}>
                ...
                {taggableEmployees.map((e) => <option key={e.id} value={e.id}>{STATUS_EMOJI[e.effective_status] || '🟢'} {e.full_name}{e.effective_status === 'On Leave' ? ' — On Leave' : ''}</option>)}
              </select>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button className="btn" style={{ background: 'var(--red)', color: '#fff' }} onClick={doDelete} disabled={deleting}>
      ...
      >
        <p style={{ fontSize: 13, color: 'var(--text3)' }}>
          This permanently deletes <b>{ticket.ticket_ref}</b> — {ticket.subject} — along with every reply,
          voice note, and attachment in it. This can't be undone.
        </p>
      </Modal>
    </div>
  );
