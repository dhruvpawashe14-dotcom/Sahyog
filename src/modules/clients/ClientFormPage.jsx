...
const ID_DOCS = [
  ...
  { key: 'aadhaar_number', label: 'Aadhaar', docType: 'Aadhaar Card', hint: 'Has front & back — select both files together, or attach one then the other' },
  { key: 'passport_number', label: 'Passport Number', docType: 'Passport', uppercase: true },
  { key: 'dl_number', label: 'Driving License', docType: 'Driving License', hint: 'Has front & back — select both files together, or attach one then the other' },
  { key: 'voter_id', label: 'Voter ID', docType: 'Voter ID', hint: 'Has front & back — select both files together, or attach one then the other' },
...
export default function ClientFormPage() {
  ...
  const totalFileCount = Object.values(idFiles).reduce((sum, arr) => sum + arr.length, 0) + otherFiles.length;
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const doc of ID_DOCS) {
        for (const file of (idFiles[doc.key] || [])) {
          await uploadDocument({ clientId: client.id, docType: doc.docType, file, uploadedBy: user.id, uploadedName: profile.full_name });
        }
      }
        ...
        <div className="form-grid">
          {ID_DOCS.map((doc) => (
            <IDFieldWithUpload
              ...
              hint={doc.hint}
              files={idFiles[doc.key] || []}
              onFilesSelect={(newFiles) => setIdFiles((prev) => ({ ...prev, [doc.key]: [...(prev[doc.key] || []), ...newFiles] }))}
              onFileRemove={(idx) => setIdFiles((prev) => ({ ...prev, [doc.key]: (prev[doc.key] || []).filter((_, i) => i !== idx) }))}
            />
          ))}
        </div>
      </div>
