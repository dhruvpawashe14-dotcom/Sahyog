...
export default function TicketDetailPage() {
  ...
  const toggleVoiceReply = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Same fix as VoiceRecorder.jsx: MediaRecorder's webm has no duration
        // in its header, so playback shows 0:00/0:00 until this patches it in.
        const mime = recorder.mimeType || 'audio/webm';
        const rawBlob = new Blob(chunksRef.current, { type: mime });
        const durationMs = Date.now() - recordStartRef.current;
        const blob = mime.includes('webm')
  ...
  const changeStatus = async (status) => {
    try {
      ...
      const notifyTargets = [ticket.raised_by, ticket.assigned_to].filter((tid) => tid && tid !== user.id);
