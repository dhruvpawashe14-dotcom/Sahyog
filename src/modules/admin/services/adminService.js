...
// leaveFrom/leaveTo are 'YYYY-MM-DD' strings, or null to clear a scheduled leave.
export async function updateEmployeeLeave(id, leaveFrom, leaveTo) {
  const { error } = await supabase.from('profiles').update({
    leave_from: leaveFrom || null, leave_to: leaveTo || null, updated_at: new Date().toISOString(),
  }).eq('id', id);
  if (error) throw error;
}
