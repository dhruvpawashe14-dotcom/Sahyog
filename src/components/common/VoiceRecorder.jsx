  ...
  const start = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        // MediaRecorder's webm container never gets a duration written into
        // its header — every player (including our own <audio> tags) then
        // has nothing to report and shows 0:00/0:00. Patch it into the blob
        // here, once, before it's ever uploaded or played. (Only webm needs
        // this — Safari's mp4 recordings already carry proper duration.)
        const mime = recorder.mimeType || 'audio/webm';
        const rawBlob = new Blob(chunksRef.current, { type: mime });
        const blob = mime.includes('webm')
          ? await fixWebmDuration(rawBlob, secondsRef.current * 1000, { logger: false })
          : rawBlob;
